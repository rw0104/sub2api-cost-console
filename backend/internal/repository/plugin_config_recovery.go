...
func (r *pluginRepository) RecoverConfig(ctx context.Context, expected *service.PluginInstallation, encrypted string, recoveredBy *int64) error {
	...
	if _, err := tx.ExecContext(ctx, "UPDATE sub2api_plugin_installations SET config_encrypted=$2,last_error='',updated_at=NOW() WHERE id=$1", expected.ID, encrypted); err != nil {
		return err
	}
	return tx.Commit()
}
