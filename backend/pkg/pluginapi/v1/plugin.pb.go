...
type ForwardRequestStart struct {
	...
	OriginalBodyJson    []byte `protobuf:"bytes,13,opt,name=original_body_json,json=originalBodyJson,proto3" json:"original_body_json,omitempty"`
	AccountMetadataJson []byte `protobuf:"bytes,14,opt,name=account_metadata_json,json=accountMetadataJson,proto3" json:"account_metadata_json,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}
...
func (x *ForwardRequestStart) GetOriginalBodyJson() []byte {
	if x != nil {
		return x.OriginalBodyJson
	}
	return nil
}

func (x *ForwardRequestStart) GetAccountMetadataJson() []byte {
	if x != nil {
		return x.AccountMetadataJson
	}
	return nil
}
...
type ListAccountsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// account_ids remains populated for plugins built against host service API v1.
	AccountIds []int64 `protobuf:"varint,1,rep,packed,name=account_ids,json=accountIds,proto3" json:"account_ids,omitempty"`
	// accounts is available when host service API v2 or newer is negotiated.
	// Every item is a readable, non-secret view in the plugin binding scope.
...
// contains credentials (access_token / refresh_token / cookies); those are
// available only through ResolveOutboundIdentity.
type AccountInfo struct {
	...
	// metadata_json is a bounded host-produced JSON object. It contains only
	// readable fields and a normalized `subscription` object when known.
...
const file_plugin_proto_rawDesc = "" +
	...
	"\x12original_body_json\x18\r \x01(\fR\x10originalBodyJson\x122\n" +
	"\x15account_metadata_json\x18\x0e \x01(\fR\x13accountMetadataJson\x1a[\n" +
