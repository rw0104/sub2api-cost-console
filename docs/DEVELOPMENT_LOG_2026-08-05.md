...
## 19. 0.2.6 上游资产表真实性与交互修复

- “点击检测”由普通提示文字改为整格可点击、可聚焦的真实按钮；烧瓶快捷按钮继续保留。
- 检测结果明确区分等待、成功与失败，并标注为“连接测试总耗时”，避免误解为纯网络 Ping 或首 Token 延迟。
- 调度原始分不再直接套用百分制等级；界面按服务端有效基础权重归一化，同时展示原始值与权重上限，例如 `95.0`、`3.80 / 4.00`。
- 删除未采集的“恢复”占位值；状态列只展示当前异常和当前限流，并给出真实错误或限流重置说明。
- 采购费率、累计采购和号池采购成本统一标记为“配置推算”，与 usage 日志产生的真实 API 账号成本、用户计费分开展示。
- 新增上游表格计算和探测单元格回归测试，覆盖低原始分归一化、动态权重、健康状态、整格点击及失败反馈。
