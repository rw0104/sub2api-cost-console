...
> [!NOTE]
> 2026-08-15 口径修订：成本中心已统一使用 `Asia/Shanghai`（北京时间）作为“当天”和月度一次性采购的业务时区。此前本节中“用户本地自然日”的描述属于中间版本，已被本修订覆盖；滚动窗口仍按真实时间计算。
...
| “今日”统计边界 | 是 | 统一使用 `Asia/Shanghai` 自然日，不受桌面机器本地时区影响 |
...
| 今日统计 | 服务器与桌面本地自然日可能混用 | 前端固定传入北京时间零点的 RFC3339 时间 |
...
成本中心的六个图表现在按数据证据显示来源标签，空档桶只保留时间轴并绘制为断点：
...
一次性采购档案不再显示为误导性的 `0/h`，而显示“非周期费用/一次性”；持续小时费率仅由周期性档案贡献。成本中心自动刷新仍是 5/10/15/30 秒间隔可选项，不存在 20 小时或每天北京时间 20:00 的定时刷新规则。北京时间只定义自然日和月度采购归属。
...
1. 桌面端按 `Asia/Shanghai` 计算当天 `00:00:00`，即使 Windows 使用美国或其他时区也不变。
2. 转成带时区含义的 RFC3339/ISO 时间。
3. 通过 `/admin/accounts/today-stats/batch` 的可选 `start_time` 传给后端；usage 兼容查询同时显式传 `timezone=Asia/Shanghai`。
...
- remediation: 由客户端传入北京时间日零点的精确 RFC3339 `start_time`。
- optional_attack:
...
2. 在北京时间午夜前后核对“今日”请求数与数据库精确起始时间。
