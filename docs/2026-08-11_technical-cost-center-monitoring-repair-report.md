...
> [!NOTE]
> 2026-08-15 最终口径：成本中心“当天”按用户设备当前日历日计算，界面不暴露服务端时区实现。最近 1/6/24 小时等仍按精确滚动时间计算。
...
| “今日”统计边界 | 是 | 使用用户设备自然日，使用户每天看到当前日历日数据 |
...
| 今日统计 | 服务器与桌面本地自然日可能混用 | 前端传入用户设备当天零点的 RFC3339 时间 |
...
### 2026-08-15 图表与采购显示修订

成本中心的六个图表现在按数据证据显示来源标签。成功查询中没有调用的金额、Token 和次数桶显示真实 `0`；接口失败显示“无数据”；延迟没有样本时保留断点并显示“无样本”：

| 图表 | 数据来源 | 是否可视为事实 |
| --- | --- | --- |
| API 产出速率 | `usage_logs.actual_cost` | 有请求样本时是实测，移动平均是派生值 |
| 实时成本速率 | usage 成本快照 + 采购档案 | API 成本实测；采购线是配置推算 |
| 收入、成本与贡献 | Ops 或 usage 经济字段 + 汇率 | 事实与配置计算混合 |
| 请求质量 | Ops 成功、失败、429/402、切号事件 | 仅在 Ops 有样本时是实测 |
| 账号健康 | 持久经济采样的状态计数 | 仅在经济采样可用时是实测 |
| 延迟与 Token 结构 | Ops TTFT、耗时和 Token 字段 | 有对应样本时是实测，空延迟保持断点 |

一次性采购档案不再显示为误导性的 `0/h`，而显示“非周期费用/一次性”；持续小时费率仅由周期性档案贡献。成本中心自动刷新是 5/10/15/30 秒间隔可选项，不存在每天固定时刻刷新的规则。
...
旧实现把服务器“今日”与用户设备“今日”混在同一页面，因此在两者日历日期不一致时会出现日期错位和看似异常的成本总额。

修复后的调用链为：

1. 桌面端按用户设备当前时区计算当天 `00:00:00`。
2. 转成带时区含义的 RFC3339/ISO 时间。
3. 通过 `/admin/accounts/today-stats/batch` 的可选 `start_time` 传给后端；usage 兼容查询同时传入设备 IANA 时区与本地日期。
...
- observed_at: 2026-08-11
- source_type: command
- source_ref: `usage_logs.created_at` 与当时的服务器时区配置
- content_hash: n/a
- artifact_path: n/a
- repro_command: |

  ```powershell
  docker exec sub2api-cost-postgres psql -U sub2api -d sub2api -c "SELECT ROUND(SUM(COALESCE(account_stats_cost,total_cost) * COALESCE(account_rate_multiplier,1))::numeric, 3) FROM usage_logs WHERE created_at >= TIMESTAMPTZ '2026-08-11T00:00:00+08:00';"
  ```
...
- repro_steps:
  1. 让服务器时区与桌面设备时区不同。
  2. 在两者自然日不一致的时段查询今日成本。
- remediation: 由客户端传入用户设备日零点的精确 RFC3339 `start_time`。
- optional_attack:
...
2. 在用户设备午夜前后核对“今日”请求数与数据库精确起始时间。
