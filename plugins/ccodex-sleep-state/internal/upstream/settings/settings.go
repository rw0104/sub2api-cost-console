...
type Config struct {
	// Plugin adapter: explicit no-probe mode, including background refresh.
	HarvestDisabled        bool     `json:"-"`
	RequestLimitExactBytes int64    `json:"-"`
	AllowedModels          []string `json:"-"`
	RoundRobin             bool     `json:"-"`
...
func Default() Config {
	return Config{StateRefreshMode: "on_demand", RequestLimitMiB: 64, ZstdWindowMiB: 64, CompactLimitMiB: 64, EgressMode: "state", Model: Model, AccountMode: "auto", UpstreamKind: "official", Listen: "127.0.0.1:17841", Upstream: "https://chatgpt.com/backend-api/codex", Direct: true,
		ProxyURLs: []string{}, ProxyEnvs: []string{}, Subscriptions: []Source{}, ProbeSeconds: 20, ProbeRoundSeconds: 20,
...
func (c Config) Validate() error {
	...
	if !c.AllowsModel(c.SelectedModel()) {
	...
	if c.ProbeRoundSeconds != 0 && (c.ProbeRoundSeconds < 1 || c.ProbeRoundSeconds > 60) {
		return errors.New("probe round budget must be 1–60 seconds")
	}
...
// The embedding host owns its explicit allowlist. The upstream defaults are
// retained only for stand-alone configs that do not supply one.
func (c Config) AllowsModel(model string) bool {
	if len(c.AllowedModels) == 0 {
		return SupportedModel(model)
	}
	for _, allowed := range c.AllowedModels {
		if allowed == model && model != "" {
			return true
		}
	}
	return false
...
func (c Config) RequestBytes() int64 {
	if c.RequestLimitExactBytes > 0 {
		return c.RequestLimitExactBytes
	}
