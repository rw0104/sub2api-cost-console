...
**v0.3.6 更新兼容内核到上游 v0.2.8，并保留插件配置密钥持久化和宿主治理修复。** 管理员可以安装、配置、启停和卸载插件；开发者可以使用公开 SDK 自行编译、签名和分发。[查看开发指南](docs/PLUGIN_DEVELOPMENT.md)，或从 [v0.3.6 发布页](https://github.com/rw0104/sub2api-cost-console/releases/tag/v0.3.6) 下载配套 SDK 与示例。
...
进入 **「上游排行」→「新增上游」**，或点击顶部 **「账号管理」** 直接进入账号列表。
...
日常操作主要集中在四个工作区：**资产总览、上游排行、渠道号池、API 接入**。顶部「账号管理」直接进入账号列表，也可以从管理界面的导航进入分组、代理、用量或系统设置。
...
点击 **「账号管理」** 进入管理界面后，还可以从导航使用内核提供的其他管理功能，包括：
...
| 下载可以直接编译的 SDK 与示例 | [v0.3.6 开发包 ZIP](https://github.com/rw0104/sub2api-cost-console/releases/download/v0.3.6/sub2api-plugin-devkit-v0.3.6.zip)：含 SDK、proto、清单 Schema、示例 UI、打包器及依赖源码 |
...
桌面 v0.3.6 使用上游 v0.2.8 兼容内核和持久化加密密钥，插件设置保存后可跨重启读取。桌面或兼容内核升级后的首次启动会停用旧插件一次并保留配置，请确认后重新启用；普通重启不重复停用。旧密钥丢失时，停用插件、重新填写配置，在宿主界面确认备份旧密文后保存；无需重新打包或更换原有 `.s2plugin`。导入提示不认识 `failure_mode` 时，请检查实际连接的服务是否为本项目的 v2 兼容内核。
...
重复打开程序会唤回已有窗口，不会再创建一份桌面实例。正常退出或桌面异常结束后，受管内核进程也会被清理。
...
登录凭据失效且无法刷新时，客户端会清除旧的登录状态并返回登录页，请重新登录。多个请求同时失败时，登录失效提示与登录页跳转只触发一次；网络中断或刷新服务暂时不可用时保留会话，恢复连接后可重试。

### 一直停留在启动页怎么办？

如果提示 Docker 尚未启动，请打开 Docker Desktop 并等待引擎就绪。应用会检查并恢复已有的本产品数据容器，服务就绪后自动继续启动；也可以点击 **「重新检测并启动」**，无需退出整个程序。等待期间不会反复创建内核进程。

如果使用高级连接，请检查所配置的 PostgreSQL、Redis / Valkey 地址、端口和连接权限；也要留意 `18765` 端口是否被其他程序占用。完整错误日志在 **「查看技术详情」** 中，恢复操作不会重建数据库或重置密码。
...
这份介绍按 **桌面 v0.3.6** 的功能整理，内置兼容内核基线为 **v0.2.8**；可下载安装的版本以发布页为准。内核可以独立更新，因此你电脑上的内核版本可能更高，具体以「版本与更新」面板为准。
