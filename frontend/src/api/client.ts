        ...
        if (!isAuthEndpoint) {
          ...
          if (refreshToken && originalRequest && !originalRequest._retry) {
            ...
            try {
              ...
              const failure = refreshError as { status?: number; response?: { status?: number; data?: { message?: string } }; message?: string }
              ...
              if (refreshStatus !== undefined && (refreshStatus === 0 || refreshStatus === 408 || refreshStatus === 429 || refreshStatus >= 500)) {
                return Promise.reject({
                  ...
                  message: failure.response?.data?.message || (refreshError as { message?: string }).message || 'Unable to refresh the session right now. Please try again.'
                })
              }
              expireAuthSession()
